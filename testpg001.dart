...
*  
*  01-Sep-2013   Brian OHara   Added rollbacks in main update & some minor
*                              changes to terminal input. After failure of
*                              an autoInc insert, loop was not repeating,
*                              so that has been changed also.
* 
...
*                              
*  01-Sep-2013   Brian OHara   Made changes to exceptions where it was detected a 
*                              change was required.
*                              
*  01-Sep-2013   Brian OHara   Enabled both program-generated and autoinc keys to
*                              run simultaneously.
*                              
*  01-Sep-2013   Brian OHara   Generally tidied-up and added comments.
* ---------------------------------------------------------------------------------                         
*
...
const int    I_MAX_PROMPT       = 12;   // (data-entry 0 to 11)
const int    I_MAX_PARAM        = 11;   // maximum parameter number (0 to 10)
...
const int    I_INS_ROLLBACK     = 4;    // prompt nr.
const int    I_USE_AUTOINC      = 5;    // prompt nr
const int    I_MAX_UPDATES      = 6;    // prompt nr
const int    I_UPD_ROLLBACK     = 7;    // prompt nr.
...
const int    I_CORRECT_YN       = 12;   // prompt nr

const int    I_DEC_PLACES       = 2;    // decimal places for currency
const int    I_TOT_OPEN         = 0;    // total type for table
const int    I_TOT_INSERT       = 1;    // total type for table
const int    I_TOT_UPDATE       = 2;    // total type for table
...
RawServerSocket ogSocket;             // for semaphore 
...
void main() {
  ...
  ClassWrapper<int> oiClassWrap     = new ClassWrapper();
  ClassWrapper<List<List>>
                ollClassWrap        = new ClassWrapper();
  int           iInsertRollbackFreq = int.parse(lsInput[I_INS_ROLLBACK]);
  int           iUpdateRollbackFreq = int.parse(lsInput[I_UPD_ROLLBACK]);
 ...
 */
    int iRequiredCount = tFirstInstance ? 0 : -1;  // 1st instance must be first (0)
    return fUpdateControlTableColumn(sColumn: "iCount1", 
                                     iRequiredCount: iRequiredCount,
                                     iMaxCount: iInstanceMax,
                                     oiResult: oiClassWrap);
 
  }).then((bool tResult) {
    iInstanceNr = oiClassWrap.value;    // unique instance number
 ...
 */    
    ...
    if (tInsertsAborted)
      print ("**** Inserts were aborted !!! ****");
    return fInsertIntoTotalsTable(oClassTotals);
  })
  .catchError((oError) =>
    fFatal ("Main", "fInsertIntoTotalsTable (Inserts) "+
                                 "Error = ${oError}"))
  .then((bool tResult) {
    if (tResult != true)
      fFatal("Main:fInsertIntoTotalsTable", "Process failed: result = ${tResult}");
...
/*
 * wait for All instances to complete main Inserts
 */
 ...
 */
    ...
    return fProcessSqlSelect(sSql, false, ollClassWrap);    
  }).then((bool tResult) {
    if (tResult != true)
      fFatal("Main:fProcessSqlSelect:", "(random) Process failed. Result = ${tResult}");
 
    List<List> llRandKeys = ollClassWrap.value;
    if (llRandKeys == null)
 ...
 */
    return fProcessMainUpdates(iInstanceNr, iMaxUpdates, llRandKeys,
 ...
 */
    ...
    print("");
/*
 * Select Main Table (unsorted) - simply to test Select
 */
...
/*
 * Select Main Table (sorted) - simply to test Select
 */
...
/*
 * Update control row to increment count of instances completed
 */
...
async.Future<bool> fProcessMainInserts(int iInstanceNr, int iMaxIters,
                                       bool tUseAutoInc, ClassTotals
                                       oClassTotals, int iInsertRollbackFreq) {
  async.Completer<bool> oCompleter = new async.Completer<bool>();

  ClassWrapper ollClassWrap  = new ClassWrapper();      // used for Select
  ...
  fLoopMainInserts = () {
    bool tAbort = (iInsertFailTot >= 100 && iInsertFailTot > iInsertTotNr);
    
    if (iInsertTotNr >= iMaxIters || tAbort) {   // All Inserts have completed
      oStopwatch.stop;
      ...
      if (iInsertRollbackFreq > 0 || iInsertRollbacks > 0)
      ...
      if (tAbort)
        ogPrintLine.fPrintForce("fProcessMainInserts: Inserts aborted - "+
                                "${iInsertFailTot} failed inserts, "+
...
/*
 * Force a Rollback if required   (I'll likely change this significantly)
 */  
    bool tForceRollback = (iInsertRollbackFreq > 0 && (iInsertRollbacks < 
                           ((iInsertTotNr+1) / iInsertRollbackFreq).toInt()));
    if (tForceRollback) {
      sCcyBal = "1234.aa";
    }
    ...
    if (!(tUseAutoInc)) {   // Don't use autoincrement
      ...
      fInsertRowWithSequence(S_MAIN_TABLE, S_SEQUENCE_KEY_MAIN,
                             sSql, tForceRollback)
      ...
      .catchError((oError) {
        ...
        fRollback("fProcessMainInserts (1)").then((_) =>
          fLoopMainInserts());
      });
      ...
      fExecuteSql(sSql, "S_MAIN_TABLE", "fProcessMainInserts", 1, tForceRollback)      
      .then((tResult) {
        if (!(tResult)) {
          if (tForceRollback) {
            iInsertRollbacks++;
          } else {  
            print ("Insert failed (autoinc problem?)");
            iInsertFailTot++;
          }
          fLoopMainInserts();
        } else {
          iInsertTotNr++;
          iCcyInsertTot += iCcyBal;
          ClassWrapper ollClassWrap = new ClassWrapper();
          fProcessSqlSelect("SELECT LASTVAL()", false, ollClassWrap)
          .then((bool tResult) {
            List<List> llLastVal = ollClassWrap.value;
            if (llLastVal != null && llLastVal.length == 1) {
              int iKey = llLastVal[0][0];
              if (iKey > 0)
                iLastIdTot++;
            }
            fLoopMainInserts();
...
async.Future<bool> fProcessMainUpdates(int iInstanceNr, int iMaxIters,
  ...
  fLoopMainUpdates = () {
    if (iUpdateTotNr >= iMaxIters) {
      ...
      if (iUpdateRollbackFreq > 0 || iUpdateRollbacks > 0)
 ...
 */
    bool tForceRollback = (iUpdateRollbackFreq > 0 && (iUpdateRollbacks < 
                          ((iUpdateTotNr + 1) / iUpdateRollbackFreq).toInt()));
    
    fUpdateMainRow(iKey, iCcyTranAmt, tForceRollback)
    .then((tResult) {
      if (tResult) {   // Update succeeded
        iUpdateTotNr ++;
        iCcyUpdateTot += iCcyTranAmt;
      } else if (tForceRollback) {
        iUpdateRollbacks += tForceRollback ? 1 : 0;   
      } else {
        iUpdateFailTot++;
        print ("Update failed");
      }
    ...
    .catchError((oError) {
      iUpdateFailTot++;
      fRollback("fProcessMainUpdates").then((_) => 
        fLoopMainUpdates());
    });
  };
  oStopwatch.start();
 ...
 */
async.Future<bool> fUpdateMainRow(int iKey, int iCcyTranAmt, bool tRollback) {
  async.Completer<bool> oCompleter = new async.Completer<bool>();

  fCheckTransactionStatus("fUpdateMainRow", true);
  ...
  ogDb.execute("begin").then((_) {
    ...
    if (lResult == null || lResult.isEmpty || lResult.length != 1)
      throw("fUpdateMainRow: failed to Select ikey = ${iKey}");
 ...
 */
    ...
    .catchError((oError) {
      if (!(tRollback))
        print ("Error caught on Update - rolling back");
      fRollback("fUpdateMainRow")
      .then((_) {
    ...
    .then((int iRowsAffected) {
      ...
      .then((_) {
        ...
        if (oiResult != null)  // calling function wants result
          oiResult.value = iCount;   // set result for calling function
        oCompleter.complete(true);
      });
    });
  })
...
async.Future<bool> fInsertRandomKeys(int iSelectTot) {
  ...
  fLoopRandomInserts = () {
    if (iPos >= lliRandKeys.length)
      return;
    
    int iKey = lliRandKeys[iPos][0];
    String sSql = "INSERT INTO random (iKey) VALUES (${iKey})";
    return fExecuteSql(sSql, "random", "fInsertRandomKeys", 1, false)
    .then((bool tResult){
      if (tResult == true)
        iPos++;
      fLoopRandomInserts();
  ...
  fProcessSqlSelect(sSql, false, ollClassWrap)
  .then((bool tResult) {
    lliRandKeys = ollClassWrap.value;
    if (lliRandKeys == null)
      throw ("fInsertRandomKeys: Select of keys from "+
              "${S_MAIN_TABLE} failed");

    ogPrintLine.fPrintForce("${lliRandKeys.length} random row(s) selected");
    fLoopRandomInserts();
...
**** waiting for a required number (of instances) to have        ****
 ...
 */
    new async.Timer(new Duration(seconds:1), () {
      ...
      .then((bool tResult) {
        ...
        List<List> llControl = ollClassWrap.value;   // get the list
        if (llControl == null || llControl.length != 1)
          fFatal("fWaitForProcess", "Failed to sSelect control row");
        int iCount = llControl[0][0];
...
async.Future<bool> fInsertRowWithSequence (String sTableName,
    String sSequenceKey, String sSql1, bool tForceRollback) {
  ...
  .catchError((oError) {
    if (!(tForceRollback)) {
      print ("fInsertRowWithSequence: Table: ${sTableName}, "+
             "Error=${oError}");
    }
    fRollback("fInsertRowWithSequence: Table ${sTableName}");
    oCompleter.complete(false);
  });  
...
async.Future<bool> fTestConnection() {
  ...
  fExecuteSql(sSql, "Drop Table if Exists", "fTestConnection", -1, false)  
  .catchError((oError) {
    oCompleter.complete(false);
  })
  .then((bool tResult) {
...
async.Future<bool> fCreateTablesEtc(bool tClearMain) {
  ...
  const int I_MAX_KEY_AUTO  = 9000000;    // Maximum auto-generated key
  const int I_MIN_KEY_PROG  = 10011000;   // Minimum program-generated key
  const int I_MIN_KEY_AUTO  = 2001000;    // Minimum auto-generated key
  
  ClassWrapper  oiClassWrap = new ClassWrapper();
  int iMaxAutoincActual;
  int iMaxAutoincSystem;
 ...
 */
  ...
  fExecuteSql(sSql, "Create 'control'", "fCreateTablesEtc", -1, false)
  .then((bool tResult) {
    if (tResult != true)
 ...
 */
  ...
  .then((bool tResult) {
    ...
    fCheckTransactionStatus(
      "fCreateTablesEtc: Prior to delete from sequences", true);
/*
 *  Create Main table
 */
    ...
    return fExecuteSql(sSql, "Create '${S_MAIN_TABLE}'", "fCreateTablesEtc", -1, false);
  })
  .catchError((oError)
    => throw(oError))
  .then((bool tResult) {
 ...
 */
    ...
    return fExecuteSql(sSql, "Create 'sequences'", "fCreateTablesEtc", -1, false)
    .catchError((oError) =>
      throw(oError))    
    .then((bool tResult) {
      if (tResult != true)
 ...
 */
      sSql = "CREATE TABLE IF NOT EXISTS random "+
          "(iKey int Primary Key not null unique)";    
      return fExecuteSql(sSql, "Create 'random'", "fCreateTablesEtc", -1, false);
    })
    .then((bool tResult) {
      if (tResult != true)
 ...
 */
      ...
      return fExecuteSql(sSql, "Create 'totals'", "fCreateTablesEtc", -1, false);
    })
    .then((bool tResult){
      if (tResult != true)
 ...
 */
      ...
      return fExecuteSql(sSql, "sequences Delete", "fCreateTablesEtc", -1, false);
    })
    .catchError((oError) =>
      throw("Delete from 'sequences' ${oError}"))
    .then((bool tResult){
      if (tResult != true)
 ...
 */
      ...
      return fExecuteSql(sSql, "sequences", "fCreateTables", 1, false);
    })
    .catchError((oError) =>
 ...
 */
    ...
    .catchError((oError) =>
      throw("Clear 'random' table ${oError}"))
    .then((bool tResult) {
      if (tResult != true)
...
/*
 * Determine the last-key for program-generated keys
 */
      print ("Select max key from Main");
      sSql = "SELECT MAX(iKey) FROM ${S_MAIN_TABLE}";
      
      return fSelectRequiredKey(sSql, oiClassWrap, I_MIN_KEY_PROG);
    })
    .catchError((oError) =>
      throw(oError))
    .then((bool tResult) {
      if (tResult != true)
        fFatal ("fCreateTablesEtc:fSelectRequiredKey", "Select failed - sSql = $sSql");  
      
      int iLastKey = oiClassWrap.value;
          
      sSql = "INSERT INTO sequences (ikey, iLastKey, stablename) VALUES "+
              "($S_SEQUENCE_KEY_MAIN, ${iLastKey}, '${S_MAIN_TABLE}')";
    ...
    .then((bool tResult) {
      if (tResult != true)
        fFatal("fCreateTablesEtc", "failed to Insert 'sequences' row");
/*
 * Check Autoinc Key for Main Table (assume nothing else currently inserting)
 *
 * Keys less than I_MAX_KEY_AUTO for autoinc - should work.
 * If this doesn't work in any situation, then clear-table.
 */
      print ("Select max actual autoinc");
      sSql = "SELECT MAX(ikey) FROM ${S_MAIN_TABLE} WHERE "+
             "ikey < ${I_MAX_KEY_AUTO}";
      return fSelectRequiredKey(sSql, oiClassWrap, I_MIN_KEY_AUTO);
    })
    .catchError((oError) =>
      throw(oError))
    .then((bool tResult) {
      if (tResult != true)
        fFatal ("fCreateTablesEtc:fSelectRequiredKey", "Select failed - sSql = $sSql");
      iMaxAutoincActual = oiClassWrap.value;
      sSql = "SELECT NEXTVAL('${S_MAIN_TABLE}_ikey_seq')";
      return fSelectRequiredKey(sSql, oiClassWrap, I_MIN_KEY_AUTO);
    })
    .catchError((oError) =>
      throw(oError))
    .then((bool tResult) {
      if (tResult != true)
        fFatal ("fCreateTablesEtc:fGetKeyFromSelect", "Select failed - sSql = $sSql");       
      iMaxAutoincSystem = oiClassWrap.value;

      if (iMaxAutoincSystem-1 == iMaxAutoincActual) {
        print ("System autoinc value is correct (${iMaxAutoincSystem-1})");
        return true;
      }
      print ("System Autoinc values are being corrected:");
      print ("System Autoinc = ${iMaxAutoincSystem-1}");
      print ("Actual Autoinc = ${iMaxAutoincActual}");
      sSql = "SELECT SETVAL(('${S_MAIN_TABLE}_ikey_seq'), ${iMaxAutoincActual})"; 
      return fExecuteSql(sSql, S_MAIN_TABLE, "fCreateTables", 1, false);
    })
    .catchError((oError) =>
      throw(oError))      
    .then((bool tResult) {
      if (tResult != true)
        fFatal("fCreateTablesEtc", "failed to SETVAL() serial "+
               "sequence for ${S_MAIN_TABLE}");
/*
 * Insert Opening Totals 
 */
...
/*
 **** Process a SELECT statement to retrieve value for a column (eg. MAX(ikey)) ****
 */
  async.Future<bool>fSelectRequiredKey (String sSql, ClassWrapper oiClassWrap,
                                        int iDefault) {
    async.Completer<bool> oCompleter = new async.Completer<bool>();
    
    int iTableValue = 0;
    
    ClassWrapper<List<List>> ollClassWrap = new ClassWrapper();
    fProcessSqlSelect(sSql, false, ollClassWrap)
    .then((bool tResult) {
      if (tResult != true) {
        fFatal("fSelectRequiredKey", "Select failed - $sSql");
      }
      
      List<List> lliResult = ollClassWrap.value;
      if (!(lliResult == null || lliResult.isEmpty ||
          lliResult[0][0] == null)) {
        iTableValue = lliResult[0][0];
      }
      
      if (iTableValue >= iDefault) {
        oiClassWrap.value = iTableValue;
      } else {
        oiClassWrap.value = iDefault;
      }
      oCompleter.complete(true);
    });
 ...
 */
  ...
  fExecuteSql(sSql, "control", "fInsertControlRow", -1, false)
  .then((bool tResult) {
    if (tResult != true)
      oCompleter.complete(false);
    else {
      ...
      return fExecuteSql(sSql, "control", "fInsertControlRow", 1, false)
      .then((bool tResult) {
...
async.Future<bool> fExecuteSql(String sSql, String sTableName,
  ...
  .then((oResult){
    ...
    if (!(tForceRollback))
      print ("fExecuteSql:${sCalledBy} Doing Rollback. Error = ${oError}");
...
async.Future<bool> fInsertOpeningTotals(int iInstanceNr) {
  ...
  .then((bool tResult) {
    ...
    List<List> llResult = ollClassWrap.value;
...
async.Future<bool> fInsertIntoTotalsTable(ClassTotals oClassTotals) {
  ...
  fInsertRowWithSequence("totals", S_SEQUENCE_KEY_TOTALS, sSql, false)
  .catchError((oError) =>
    throw(oError))
  .then((tResult) {
    if (tResult != true)
 ...
 */
    List<List> llRows = ollClassWrap.value;
 ...
 */
    ...
    fProcessSqlSelect(sSql, false, ollClassWrap)
    .then((bool tResult) {
      List<List> llTotals = ollClassWrap.value;
      if (llTotals == null || llTotals.length < 1)
        throw ("fDisplayTotals: Select of 'totals' table failed");

      String sCcyTot2 = "0.0";
      
      if (llTotals[0][0] != null)
        iRowTot2 = llTotals[0][0];
      if (llTotals[0][1] != null)
        sCcyTot2 = llTotals[0][1];      
...
async.Future<bool> fProcessSqlSelect(String sSql, bool tShowTime,
                                     ClassWrapper<List<List>> ollClassWrap) {
  ...
  ogDb.query(sSql).toList().then((llRows) {
    ...
    if (ollClassWrap != null)
      ollClassWrap.value = llRows;
    oCompleter.complete(true);
    return;
  }).catchError((oError) =>
...
/*
 **** ClassTotals for storing total numbers and currency-values
 **** for opening values and inserts and deletes.
 */
...
/*
 * Class to wrap a Type for return value from method
 */
...
class ClassTerminalInput {
  ...
  ClassTerminalInput() {
    ...
    _lliMinMax[I_INS_ROLLBACK] = [0, 10000];   
    _lliMinMax[I_USE_AUTOINC]  = [1];
    _lliMinMax[I_MAX_UPDATES]  = [0, 1000000];
    _lliMinMax[I_UPD_ROLLBACK] = [0, 10000];    
    ...
    _lsDataTypes[I_INS_ROLLBACK]  = "int";    
    _lsDataTypes[I_MAX_UPDATES]   = "int";
    _lsDataTypes[I_UPD_ROLLBACK]  = "int";    
  ...
  String _fReadConsoleLine(int iPromptNr, String sDefault,
    ...
    while (true) {
      ...
      while (iInput != 10) {   // 10 = newline
        // I'll likely alter this to read raw bytes when available //
      ...
      String sErrorLine = "";      
      try {
        if (sTerminate != null && (sInput == sTerminate ||
            sInput.toLowerCase() == sTerminate)) {
...
////    int iInput = int.parse(sInput, onError: (_) {
 ...
 */
  int fValidateAllInput(List lsData, int iMaxPrompt) {
    ...
    return iMaxPrompt +1;   // all entries are valid
  } 
